...
{
	if(current_process->burstTime == INIT_BURST_TIME)
	{
		return;
	}
...
char haveToChangeProcess()
{
	...
	int temp_burstTime = current_process->burstTime;

    do {
    	if(temp_pcb->waiting_time >= WAITING_LIMIT || temp_pcb->burstTime <= temp_burstTime) {
    		return TRUE;
    	}
   		temp_pcb = temp_pcb->next_pcb;
...
void elect() {
	...
	// initial values
	int shortestBurstTime = current_process->burstTime;
	...
	do {
		...
		else if(temp_pcb->burstTime <= shortestBurstTime) {
			shortestBurstTime = temp_pcb->burstTime;
			current_process = temp_pcb;
		}
		temp_pcb = temp_pcb->next_pcb;
